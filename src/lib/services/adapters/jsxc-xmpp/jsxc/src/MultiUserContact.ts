...
export default class MultiUserContact extends Contact {
   ...
   public async invite(jid: JID, reason?: string) {
       const discoInfo = await this.account.getDiscoInfoRepository().requestDiscoInfo(this.getJid());
       const isMembersOnly = discoInfo.hasFeature('muc_membersonly');
       ...
       const resources = jid.resource ? [jid.resource] : contact.getResources();

       if (resources.length > 0) {
         for (const resource of resources) {
            if (contact.hasFeatureByResource(resource, 'jabber:x:conference')) {
                const password = this.data.get('password');
   ...
   public unban(target: IJID, reason?: string) {
      return this.getService().unbanUser(this.getJid(), target, reason);
   }
   ...
   public rejectInvitation() {
      return this.getService().declineMediatedMultiUserInvitation(this.account.getJID(), this.getJid())
   }
   ...
   public createPreconfiguredRoom() {
      ...
      const form = Form.fromJSON(this.getRoomConfiguration());
   ...
   /* public getChatRootElement(): MultiUserChatWindow {
      if (!this.chatRootElement) {
         this.chatRootElement = new MultiUserChatWindow(this);
      }

      return this.chatRootElement;
   } */

   public getMember(nickname: string): { affiliation?: AFFILIATION; role?: ROLE; jid?: JID } {
       const data = this.getMembers().get(nickname) || {};
   ...
   public addMember(nickname: string, affiliation?: AFFILIATION, role?: ROLE, jid?: JID): boolean {
       const isNewMember = !this.getMembers().get(nickname);
   ...
   public getRoomConfiguration() {
       const roomConfig = this.data.get('roomConfig');
   ...
   public getJoinDate(): Date {
       const dateString = this.data.get('joinDate');
