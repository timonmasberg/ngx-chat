...
export default class UserMedia {
    ...
    private static async filterUserMedia(userMedia: string[]): Promise<string[]> {
        ...
        userMedia = userMedia.filter(el => devices[el + 'input']?.length > 0);
    ...
    private static async getMediaDevices(): Promise<{ audioinput: string[]; videoinput: string[] }> {
        ...
        const mediaDevices = devices.reduce(
            (mediaDevicesCollection, device) => {
                mediaDevicesCollection[device.kind]?.push(device.deviceId);

                return mediaDevicesCollection;
            },
            {
    ...
    private static async getUserMedia(um) {
        ...
        while (
            ...
            try {
                ...
                Log.assert(um.includes('video'), `Using ${devices.videoinput[videoIndex]} as video device`);
                Log.assert(um.includes('audio'), `Using ${devices.audioinput[audioIndex]} as audio device`);
