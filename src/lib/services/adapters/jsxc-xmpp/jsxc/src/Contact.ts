...
export default class Contact implements IIdentifiable, IContact {
    ...
    public static getProviderId(account: Account, id: string) {
        const data = new PersistentMap(account.getStorage(), 'contact', id);
    ...
    public getChatRootElement(): Element {
    ...
    public getChatWindowController(): ChatWindowControllerInterface {
        if (!this.chatWindowController) {
            this.chatWindowController = {
                close: () => {
                },
                minimize: () => {
                },
    ...
    public hasFeatureByResource(resource, feature) {
        ...
        const jid = new JID(this.jid.bare + '/' + resource);
    ...
    public getLastMessageDate(): Date {
        const lastMessage = this.data.get('lastMessage');
