...
export default class IceServers {
   public static isExpiring(): boolean {
      const rtcPeerConfig = Client.getOption<IRTCPeerConfig>('RTCPeerConfig') || ({} as IRTCPeerConfig);
   ...
   public static get(): Promise<ICEServer[]> {
      ...
      const url = rtcPeerConfig.url;

      const ttl = (storage.getItem('iceValidity') || 0) - new Date().getTime();
      ...
      return new Promise((resolve, reject) => {
         $.ajax(url, {
            ...
            .done((data: IRTCPeerConfig) => {
               const peerConfig: IRTCPeerConfig = Client.getOption('RTCPeerConfig');
               const ttl = data.ttl || peerConfig.ttl || 3600;
               const iceServers = data.iceServers;

               if (iceServers && iceServers.length > 0) {
                  const urls = iceServers[0].urls && iceServers[0].urls.length > 0;
   ...
   private static startRenewalTimeout() {
      const validity = Client.getStorage().getItem('iceValidity');

      if (!validity) {
         return;
      }

      const ttl = validity - new Date().getTime();
