...
export default class PresenceController {
   ...
   public getTargetPresence(): Presence {
       const extPresence = this.storage.getItem(TARGET_KEY);
       const presence = extPresence && typeof extPresence === 'object' ? extPresence.presence : extPresence;
   ...
   public getCurrentPresence(): Presence {
       const presence = this.storage.getItem(CURRENT_KEY);
   ...
   public registerAccount(account: Account) {
       const sessionStorage = account.getSessionStorage();
   ...
   private updateCurrentPresence() {
       const commonPresence = this.getCommonPresence();
   ...
   private getCommonPresence(): Presence | false {
      ...
      accounts.forEach(account => {
          const presence = account.getPresence();
