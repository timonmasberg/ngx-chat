...
class Registration {
    ...
    public requestForm(callback: (form: Form) => Promise<Form>) {
        ...
        return this.connection
            .sendIQ(queryStanza)
            .then(iqStanza => {
                const form = this.getFormFromResult(iqStanza);
    ...
    private getFormFromResult = iqStanza => {
        const queryStanza = iqStanza.find(`query[xmlns="${NS_REGISTER}"]`);
        ...
        const formStanza = queryStanza.find(`x[type="form"][xmlns="jabber:x:data"]`);
        ...
        for (const allowedField of ALLOWED_FIELDS) {
        ...
        const redirection = queryStanza.find('>x[xmlns="jabber:x:oob"] url').text();
...
class Connection {
    ...
    public send(childStanza: Strophe.Builder): Promise<JQuery> {
        const stanza = $build('body', this.getBodyAttributes());
    ...
    public sendIQ(childStanza: Strophe.Builder, type: 'get' | 'set' = 'get'): Promise<JQuery> {
        ...
        return this.send(iq).then(stanza => {
            const iqResultStanza = $(stanza).find(`iq[type="result"][id="${id}"]`);
