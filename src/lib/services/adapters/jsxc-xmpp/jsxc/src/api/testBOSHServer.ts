...
export function testBOSHServer(url: string, domain: string): Promise<string> {
   const rid = '123456';
   const requestString = `<body rid='${rid}' xmlns='http://jabber.org/protocol/httpbind' to='${domain}' xml:lang='en' wait='60' hold='1' content='text/xml; charset=utf-8' ver='1.6' xmpp:version='1.0' xmlns:xmpp='urn:xmpp:xbosh'/>`;
   ...
   return new Promise((resolve, reject) => {
      $.ajax({
         ...
         .done(stanza => {
            try {
               const result = processResponse(stanza);
...
function processResponse(stanza) {
   ...
   const body = $(stanza).find('body[xmlns="http://jabber.org/protocol/httpbind"]');
   const condition = body ? body.attr('condition') : null;
   const type = body ? body.attr('type') : null;
