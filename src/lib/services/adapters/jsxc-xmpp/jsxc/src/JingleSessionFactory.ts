...
export default class JingleSessionFactory {
   public static create(account: Account, session: IOTalkJingleSession) {
      const sessionType = JingleSessionFactory.getSessionType(session);
   ...
   private static determineMediaSessionType(session: IOTalkJingleSession) {
      let reqMedia = false;
      const description = session.isInitiator ? session.pc.localDescription : session.pc.remoteDescription;

      description.contents.forEach(content => {
         const audioOrVideoRequested =
