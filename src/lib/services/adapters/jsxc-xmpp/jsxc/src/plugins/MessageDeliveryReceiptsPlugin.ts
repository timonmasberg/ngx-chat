...
export default class ReceiptPlugin extends AbstractPlugin {
    ...
    constructor(pluginAPI: PluginAPI) {
        ...
        NS.register('RECEIPTS', 'urn:xmpp:receipts');
        pluginAPI.addFeature(NS.get('RECEIPTS'));
    ...
    private preSendMessageStanzaProcessor = (message: Message, xmlStanza: Strophe.Builder): Promise<any> => {
        ...
        return discoRepository.hasFeature(message.getPeer(), [NS.get('RECEIPTS')]).then(hasFeature => {
            if (hasFeature) {
                addRequest();
            }
