...
export default class Bootstrap {
   ...
   public async prepare(): Promise<void> {
      const bundle = await this.bundleManager.refreshBundle();
      if (!this.store.isPublished()) {
         ...
         const bundle = await this.bundleManager.generateBundle(identityKey);
         const deviceId = this.store.getLocalDeviceId();
