...
import {ArgumentError, OnlyGroupChatError} from '../CommandRepository';
...
function showCommandHelp(help: { label: string; commands: { command: string; description: string }[] }[]) {}
...
export default class CommandPlugin extends AbstractPlugin {
    ...
    private register() {
        ...
        this.pluginAPI.registerCommand(
            '/clear',
            async (_, contact) => {
                // contact.getChatRootElement().clear();
                contact.getChatRootElement().remove();
