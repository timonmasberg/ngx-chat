...
export default class AvatarPEPPlugin extends AbstractPlugin {
   ...
   private publishAvatarProcessor = async (avatar: IAvatar | null): Promise<[IAvatar]> => {
      ...
      if (!avatar || !avatar.getData()) {
         const item = $build('metadata', { xmlns: UANS_METADATA });
      ...
      const dataElement = $build('data', { xmlns: UANS_DATA }).t(imageData).tree();

      await pepService.publish(UANS_DATA, dataElement, imageId);

      const imageSize = FileHelper.getFileSizeFromBase64(imageDataUrl);

      const metadataElement = $build('metadata', {
         xmlns: UANS_METADATA,
      })
         .c('info', {
            bytes: imageSize,
   ...
   private onMessageAvatarUpdate = stanza => {
      ...
      const metadata = $(stanza).find(`metadata[xmlns="${UANS_METADATA}"]`);
   ...
   private avatarProcessor = async (contact: IContact, avatar: IAvatar): Promise<[IContact, IAvatar]> => {
      ...
      const meta = storage.getItem<AvatarMetaData>(contact.getJid().bare);
