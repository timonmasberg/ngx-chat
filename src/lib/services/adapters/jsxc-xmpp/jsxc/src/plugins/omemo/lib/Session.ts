...
export default class Session {
   ...
   public async decrypt(ciphertext, preKey: boolean = false): Promise<ArrayBuffer> {
       const sessionCipher = this.getSessionCipher();
       let plaintextBuffer;
   ...
   public async encrypt(plaintext): Promise<EncryptedDeviceMessage | null> {
      try {
         ...
         const session = this.getSessionCipher();
         const ciphertext = await session.encryptMessage(plaintext);
   ...
   private async establishSession() {
       const bundle = await this.bundleManager.requestBundle(this.address);

       await this.processPreKeyMessage(bundle);
   }

   private processPreKeyMessage = (bundle: Bundle): Promise<void> => {
       const builder = new SessionBuilder(this.address, this.store);
