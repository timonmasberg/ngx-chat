...
export default class OMEMOPlugin extends EncryptionPlugin {
    ...
    public static updateEncryptionState(contact: IContact, trust: Trust) {
        const state = trust === Trust.confirmed ? EncryptionState.VerifiedEncrypted : EncryptionState.UnverifiedEncrypted;
   ...
   /* private openDeviceDialog = async (chatWindow: ChatWindow) => {
        const peerContact = chatWindow.getContact();
    ...
    public toggleTransfer(contact: IContact): Promise<void> {
        ...
        return this.getOmemo()
            ...
            .then(async () => {
                ...
                const trust = this.getOmemo().getTrust(contact);
    ...
    private async refreshDeviceList(contact: IContact) {
        const pepService = this.pluginAPI.getConnection().getPEPService;

        try {
            const stanza = await pepService.retrieveItems(NS_DEVICELIST, contact.getJid().bare);
    ...
    private onDeviceListUpdate = stanza => {
        const messageStanza = $(stanza);
        const itemsElement = messageStanza.find(`items[node="${NS_DEVICELIST}"]`);
        const listElement = messageStanza.find(`list[xmlns="${NS_BASE}"]`);
        const fromString = messageStanza.attr('from');
        ...
        const fromJid = this.pluginAPI.createJID(fromString);
    ...
    private processEncryptedAttachment(plaintext: string, message: IMessage, contact: IContact) {
        ...
        const matches = lines[0].match(AESGCM_REGEX);
        ...
        const [match, , filename, extension] = matches;
        const mime = /^(jpeg|jpg|gif|png|svg)$/i.test(extension)
            ? `image/${extension.toLowerCase()}`
            : 'application/octet-stream';
        const attachment = new Attachment(decodeURIComponent(filename), mime, match);
    ...
    private preSendMessageProcessor = async (contact, message) => {
        ...
        const encryptedFile = await this.encryptFile(attachment.getFile());
    ...
    private async encryptFile(file: File) {
        ...
        (encryptedFile as any).aesgcm = ArrayBufferUtils.toHex(iv) + ArrayBufferUtils.toHex(keydata);
    ...
    private preSendMessageStanzaProcessor = async (message: IMessage, xmlElement: Strophe.Builder) => {
        ...
        const isTrustUnknown = this.getOmemo().isTrustUnknown(contact);
    ...
    private handleNewDevice(contact: IContact): Promise<void> {
        return Promise.resolve();
         const overlayElement = chatWindow.getOverlay();

        overlayElement.append('<p>' + Translation.t('There_are_new_devices') + '</p>');

        const continueButton = $('<button class="jsxc-button jsxc-button--block jsxc-button--primary" />');
        continueButton.text(Translation.t('Configure'));
        overlayElement.append(continueButton);

        const cancelButton = $('<a href="#" class="jsxc-button jsxc-button--block" />');
    ...
    private isLibSignalAvailable() {
        return typeof (window as any).libsignal !== 'undefined';
    }
}
