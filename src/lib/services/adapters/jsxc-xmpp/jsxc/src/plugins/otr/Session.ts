...
export default class Session {
    ...
    public processMessage(message: Message, type: 'decryptMessage' | 'encryptMessage'): Promise<Message> {
        const plaintextBody = message.getPlaintextMessage();
    ...
    private encryptMessage(message: Message): Promise<Message> {
        const self = this;
        const messageId = message.getUid();
    ...
    private statusHandler = status => {
        switch (status) {
            ...
            case OTR.CONST.STATUS_AKE_SUCCESS:
                const msgState = this.session.trust ? 'Verified' : 'Unverified';
    ...
    private smpHandler = (type: string, data: boolean | string) => {
        switch (type) {
            case 'question': // verification request received
                ...
                // const dialog = this.openVerificationDialog();
    ...
    private restoreSession = payload => {
        ...
        const encryptionState = this.peer.getEncryptionState();
    ...
    private saveSession() {
        const payload: any = {}; // return value
