...
export default class OTRPlugin extends EncryptionPlugin {
   ...
   constructor(pluginAPI: PluginAPI) {
      ...
      pluginAPI.registerChatRootElementInitializedHook((chatRootElement: Element) => {
        /* const contact = chatWindow.getContact();

         if (contact.getType() !== ContactType.CHAT) {
            return;
         }

         const menuEntry = chatWindow.addMenuEntry('otr-verification', 'OTR ' + Translation.t('Verification'), () =>
   ...
   private getDSAKey() {
      ...
      const storage = this.pluginAPI.getStorage();
      const storedKey = storage.getItem('key');
      ...
      if (!storedKey) {
        ...
        // this.key = (DSA as IDSA).parsePrivate(storedKey);
   ...
   private generateDSAKey(): Promise<{}> {
      ...
      return new Promise((resolve, reject) => {
         ...
         /* (DSA as IDSA).createInWebWorker(
            {
               path: '',
                  // typeof dsaWebworkerFile === 'string'
                  // ? dsaWebworkerFile
                  // : dsaWebworkerFile && (dsaWebworkerFile as any).default,
            },
            key => {
