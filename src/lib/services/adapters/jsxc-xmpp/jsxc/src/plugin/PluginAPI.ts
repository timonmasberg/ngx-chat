...
import CommandRepository, {CommandAction} from '../CommandRepository';
import CallManager from '../CallManager';
import IMenuItemFactory from '../MenuItemFactory.interface';
...
export default class PluginAPI implements IPluginAPI {
   ...
   public getSessionStorage(): IStorage {
      if (typeof this.sessionStorage === 'undefined') {
         const name = this.account.getUid() + '@' + this.account.getSessionId() + '@' + this.name;
   ...
   public getConnectionCreationDate(): Date {
      ...
      const created = storage.getItem('connection', 'created');
   ...
   public registerChatRootElementInitializedHook(hook: (chatRootElement?: Element, contact?: Contact) => void) {
      this.account.registerChatWindowInitializedHook(hook);
   }

   public registerChatWindowClearedHook(hook: (chatRootElement?: Element, contact?: Contact) => void) {
   ...
   public registerTextFormatter(
      ...
      Message.addFormatter((text: string, direction: DIRECTION, peer: IJID, senderName: string) => {
         return Promise.resolve(formatter(text, direction, this.account.getContact(peer), senderName)).then((innerText) => [
            innerText,
            direction,
            peer,
            senderName,
