...
export default class PluginRepository {
   ...
   constructor(private account: Account) {
      const accountDisabledPlugins = account.getOption('disabledPlugins');
   ...
   public getAllEnabledRegisteredPlugins() {
      const disabledPlugins = Client.getOption('disabledPlugins') || [];
   ...
   public getEncryptionPlugin(pluginId: string): EncryptionPlugin {
      for (const plugin of this.encryptionPlugins) {
         if ((plugin.constructor as IPlugin).getId() === pluginId) {
            return plugin;
         }
      }
   ...
   public getPlugin<TPlugin extends AbstractPlugin>(pluginId: string): TPlugin {
