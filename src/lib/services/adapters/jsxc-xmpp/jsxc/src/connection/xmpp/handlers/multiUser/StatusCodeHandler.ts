...
export default class MultiUserStatusCodeHandler {
   public static processCodes(codes: string[], multiUserContact: MultiUserContact, nickname?: string) {
       const statusCodeHandler = new MultiUserStatusCodeHandler(multiUserContact, codes.indexOf('110') > -1, nickname);

       for (const code of codes) {
          const msg = statusCodeHandler.processCode(code);
   ...
   private 201() {
       const multiUserContact = this.multiUserContact;
      ...
      showSelectionDialog({
         ...
         primary: {
            ...
            cb: () => {
               ...
               const instantRoomPromise = multiUserContact.createInstantRoom();
         ...
         option: {
            ...
            cb: () => {
                const roomConfigurationPromise = showRoomConfigurationDialog(multiUserContact);
