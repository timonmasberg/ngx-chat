...
export default class XMPPConnection extends AbstractConnection implements IConnection {
   ...
   private processPendingStorageConnections() {
       ...
       const pendingStanzaItems = storage.getItemsWithKeyPrefix(STANZA_KEY);
       const pendingStanzaIQItems = storage.getItemsWithKeyPrefix(STANZA_IQ_KEY);

       for (const key in pendingStanzaItems) {
         this.onIncomingStorageStanza(key, pendingStanzaItems[key]);
      }

       for (const key in pendingStanzaIQItems) {
   ...
   private onIncomingStorageStanza(key: string, stanzaString: string) {
       const stanzaElement = new DOMParser().parseFromString(stanzaString, 'text/xml').documentElement;
