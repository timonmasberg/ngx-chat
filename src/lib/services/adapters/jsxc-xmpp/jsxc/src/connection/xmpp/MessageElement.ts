...
export class MessageElement {
   ...
   private direction = Message.DIRECTION.IN;
    ...
    private findMessageElement(stanza: Element) {
        const forwardedStanza = $(stanza).find('forwarded' + NS.getFilter('FORWARD'));

        const from = new JID($(stanza).attr('from'));
        const to = new JID($(stanza).attr('to'));
        ...
        const carbonStanza = $(stanza).find('> ' + NS.getFilter('CARBONS'));
        ...
        if (from.bare === to.bare) {
            const carbonTagName = (carbonStanza.prop('tagName') as string) || '';
    ...
    public getStanzaId() {
        // @REVIEW "by" attribute ?
        const stanzaIdElement = this.element.find('stanza-id[xmlns="urn:xmpp:sid:0"]');

        return stanzaIdElement.attr('id');
    }

    public getTime() {
        const delayElement = this.element.find('delay[xmlns="urn:xmpp:delay"]');
   ...
   public getPlaintextBody() {
       const body = Utils.removeHTML(this.element.find('> body').text());
