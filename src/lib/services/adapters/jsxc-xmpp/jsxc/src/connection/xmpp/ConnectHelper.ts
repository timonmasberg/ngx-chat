...
function prepareConnection(url: string): Strophe.Connection {
   const connection = new Strophe.Connection(url, {
      mechanisms: [
         (Strophe as any).SASLAnonymous,
         (Strophe as any).SASLExternal,
         (Strophe as any).SASLPlain,
         (Strophe as any).SASLSHA1,
      ],
   } as any);
