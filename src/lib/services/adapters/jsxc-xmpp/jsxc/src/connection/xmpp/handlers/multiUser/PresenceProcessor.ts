...
export default class MultiUserPresenceProcessor {
   ...
   private processNewMember() {
       const itemElement = this.xElement.find('item').first();
       const jidString = itemElement.attr('jid');
       const affiliation = itemElement.attr('affiliation');
       const role = itemElement.attr('role');

       const jid = jidString ? new JID(jidString) : undefined;

       const isNew = this.multiUserContact.addMember(this.nickname, affiliation, role, jid);
