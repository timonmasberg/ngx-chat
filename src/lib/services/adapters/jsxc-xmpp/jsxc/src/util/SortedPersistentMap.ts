...
export default class SortedPersistentMap {
   ...
   private onStorage = (newValue: any, oldValue: any, key: string) => {
      ...
      const pushDiff: string[] = newValue.filter(id => oldValue.indexOf(id) < 0);

      for (const value of pushDiff) {
         // call push hook
         if (typeof this.pushHook === 'function') {
             const result = this.pushHook(value);
      ...
      const removeDiff: string[] = oldValue.filter(id => newValue.indexOf(id) < 0);

      for (const removeValue of removeDiff) {
         // call remove hook
         if (typeof this.removeHook === 'function') {
            this.removeHook(removeValue, this.map[removeValue]); // @REVIEW key is missing
         }

         delete this.map[removeValue];
      }
