...
export class Logger implements ILog {
    ...
    public assert(shouldLog: boolean, message: string, ...data): void {
        if (shouldLog) {
    ...
    private log(level: LogLevel, message: string, ...data) {
        const args = [this.getPrefix(level) + message, ...data];
        ...
        if (typeof console !== 'undefined') {
            ...
            const logFunction = level === LogLevel.Warn || level === LogLevel.Error ? 'warn' : 'log';
